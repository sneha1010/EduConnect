...
class Column(Div):
    ...
    Depending on the template, styles class associated to the div is formColumn, row, or nothing. For this last case, you
     must provide styles classes. Example::
...
class MultiWidgetField(Field):
    ...
    .. note:: To override widget's styles class use ``class`` not ``css_class``.
    """
